...
with tab1:
    ...
    with col_a:
        ...
        cols_div = ["Symbol", "Name", "시총", "주가", "월배당금", "1M배당률", "3M수익률", "배당일"]
        ...
        st.dataframe(
            top_div[cols_div].style.format(fmt_div),
    ...
    with col_b:
        ...
        cols_ret = ["Symbol", "Name", "시총", "주가", "3M수익률", "1Y수익률"]
        ...
        st.dataframe(
            top_ret[cols_ret].style.format(fmt_ret),
            use_container_width=True,
            height=600,
...
with tab2:
    ...
    edited = st.data_editor(
        ...
        column_config={
            "ETF 종목": st.column_config.SelectboxColumn(
                "ETF 종목", options=etf_names, required=True
            ),
            "투자금 (원)": st.column_config.TextColumn("투자금 (원)"),
        },
        num_rows="dynamic",
        use_container_width=True,
    )
    ...
    if "simul_result" in st.session_state:
        ...
        c4.metric("주가차익(3M)", f"{total_profit_3m:,.0f}원")
